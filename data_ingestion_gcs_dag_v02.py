...
OUTPUT_FILE_TEMPLATE = 'output_yellowtaxi_{{ execution_date.strftime(\'%Y-%m\') }}.csv'
parquet_file = OUTPUT_FILE_TEMPLATE.replace('.csv', '.parquet')


def format_to_parquet(src_file ,ti):
    if not src_file.endswith('.csv'):
        logging.error("Can only accept source files in CSV format, for the moment")
        return
    table = pv.read_csv(src_file)
    pq.write_table(table, src_file.replace('.csv', '.parquet'))
    ti.xcom_push(key='parquet', value=src_file[:-3]+'parquet')
...
def upload_to_gcs(bucket, object_name, ti):
    ...
    blob.upload_from_filename(ti.xcom_pull(key='parquet', task_ids='format_to_parquet_task'))
